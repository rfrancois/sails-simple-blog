...
module.exports = function login(inputs) {
  ...
  // Look up the user
  User.attemptLogin({
    email: sails.he.encode(req.param('email')),
    ...
    if (err || !user) {
      ...
      return res.badRequest({
        ...
        notification: {
          type: "error",
          message: 'La combinaison nom et mot de passe n\'est pas bonne.'
        }
      }, {view: "user/login"});
    }
    if(user.active == 0) {
    ...
    if(req.session.rememberPage) {
      inputs.successRedirect = req.session.rememberPage;
      req.session.rememberPage = undefined;
    }
